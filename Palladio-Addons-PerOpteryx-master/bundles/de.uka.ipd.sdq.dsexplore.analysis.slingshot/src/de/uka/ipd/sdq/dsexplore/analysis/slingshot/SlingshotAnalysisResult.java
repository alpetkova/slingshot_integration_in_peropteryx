...
public class SlingshotAnalysisResult extends AbstractPerformanceAnalysisResult implements IStatisticAnalysisResult, IPerformanceAnalysisResult {
	...
	private final SlingshotQualityAttributeDeclaration qualityAttributeInfo;
	...
	protected SlingshotAnalysisResult(final ExperimentRun run, final PCMInstance pcmInstance, final Entity pcmEntity, final Map<Criterion, EvaluationAspectWithContext> objectiveToAspect,
			final SlingshotQualityAttributeDeclaration qualityAttributeInfo) throws AnalysisFailedException {
		super(pcmInstance);
		...
		SlingshotAnalysisResult.logger.debug("Initialised Slingshot EDP2 result");
	}
	...
	private double[] getResponseTimesOfScenario() {
		...
		if (measurements.size() != 1) {
			SlingshotAnalysisResult.logger.error("Invalid amount of measurement points for scenario have been found!");
			return null;
		}
	...
	private void retrievePassiveResourceUtil(ResultDecoratorRepository repo, PCMInstance pcmInstance) {
		...
		SlingshotAnalysisResult.logger.warn("Passive Resources will not monitored so far");
	}

	private void retrieveServiceResults(PCMInstance pcmInstance, ResultDecoratorRepository repo) {
		// TODO Implement me
		// See also SimuComAnalysisSensorFrameworkResult
		SlingshotAnalysisResult.logger.warn("Services will not monitored so far");
	}
	...
	private void getUtilisationOfResource(final ActiveResourceUtilisationResult resultToFill, final Entity container, final ResourceType resourceType) {
		...
		if (!(resourceType instanceof ProcessingResourceType)) {
			SlingshotAnalysisResult.logger.warn("Only CPU measurements are currently supported ..");
			resultToFill.setResourceUtilisation(Double.NaN);
			return;
		}
	...
	public double getValueFor(Criterion criterion) {
		...
		SlingshotAnalysisResult.logger.warn("Unknown aspect for simu com result, adding NaN.");
		return Double.NaN;
	}
	...
	public static boolean isExperimentRunExisting(final String experimentName, final String experimentSettingName, Repository repo) {
		return (SlingshotAnalysisResult.findExperimentRun(experimentName, experimentSettingName, repo) != null);
	}
	...
	public static IStatisticAnalysisResult findExperimentRunAndCreateResult(Entity entity, String experimentName, String experimentSettingName, PCMInstance pcmInstance, Repository repo,
			Map<Criterion, EvaluationAspectWithContext> criterionToAspect, SlingshotQualityAttributeDeclaration qualityAttribute) throws AnalysisFailedException {

		ExperimentSetting mySetting = SlingshotAnalysisResult.findExperimentRun(experimentName, experimentSettingName, repo);
		if (mySetting != null) {
			...
			return new SlingshotAnalysisResult(reqRun, pcmInstance, entity, criterionToAspect, qualityAttribute);
		}
